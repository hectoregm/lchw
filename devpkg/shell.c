...
int Shell_exec(Shell template, ...)
{
        ...
        check(template.vargs <= args_replaced, "Failed to replace needed arguments.");
...
int Shell_run(apr_pool_t *p, Shell *cmd)
{
        ...
        check(cmd->exit_code == 0, "%s exited baldy.", cmd->exe);
...
Shell CLEANUP_SH = {
        .exe = "rm",
        .dir =  "/tmp",
        .args = {"rm", "-rf", "/tmp/pkg-build", "/tmp/pkg-src.ta.gz",
